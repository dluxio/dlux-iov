...
export default {
  ...
  <div class="d-flex flex-column">
    ...
    <!-- Upload Interface -->
    <div v-if="contractBuilt" class="mt-3">
      <upload-vue
        :user="saccountapi"
  ...
  methods: {
    ...
    handleModalSign(op) {
      this.loading = true;
      this.$emit('tosign', op);
      // Simulate contract creation confirmation (in reality, wait for blockchain confirmation)
      setTimeout(() => {
        this.fetchLatestContract();
      }, 2000); // Adjust delay as needed
    },
    fetchLatestContract() {
      // Fetch user's contracts to find the latest one
      fetch(`${this.sapi}/@${this.account}`)
        .then((response) => response.json())
        .then((data) => {
          const latestContract = data.file_contracts && data.file_contracts.length > 0
            ? data.file_contracts[0] // Assuming sorted by creation time
            : { i: 'mockContractId', b: 'mockBroker' }; // Mock for demo
          this.selectedContract = {
            i: latestContract.i,
            b: latestContract.b, // Broker
            t: this.account,    // Owner
            a: this.totalSize,  // Available size
          };
        ...
        .catch((error) => {
          ...
          this.loading = false;
          // Mock fallback
          this.selectedContract = { i: 'mockContractId', b: 'mockBroker', t: this.account, a: this.totalSize };
          this.contractBuilt = true;
        });
    },
    sendIt(event) {
