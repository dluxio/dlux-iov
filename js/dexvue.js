...
var app = new Vue({
  ...
  methods: {
    ...
    setApi(url) {
      ...
      url = url || prompt("Please enter your API", "https://spkgiles.hivehoneycomb.com");
    ...
    getHiveUser(user) {
      if (user)
        ...
        if (localStorage.getItem(`hhp:${user}`) && 
          ...
          fetch("https://hive-api.dlux.io", {
            body: `{"jsonrpc":"2.0", "method":"condenser_api.list_proposal_votes", "params":[["${user}", 322], 1, "by_voter_proposal", "ascending", "active"], "id":1}`,
            headers: {
              "Content-Type": "application/x-www-form-urlencoded",
            },
            method: "POST",
