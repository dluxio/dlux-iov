...
function isChrome() {
  ...
  if (isIOSChrome) {
    ...
    AFRAME.scenes[0].emit("updateText", {
      lineOne: "Welcome to dlux",
      lineTwo: "Mobile Chrome is untested.",
      lineThree: "Please use firefox",
    });
    ...
    AFRAME.scenes[0].emit("updateText", {
      lineOne: "Welcome to dlux",
      lineTwo: "Share anything!",
      lineThree: "Earn rewards!",
    });
  } else {
    // Not Chrome
...
AFRAME.registerComponent("trending", {
  //trending="tag" to pull another tag, default delux
  schema: { default: "" },
  init: function () {
    this.el.addEventListener("click", function () {
...
AFRAME.registerComponent("new", {
  //new="username" to pull user feeds, defalt dlux tag
  schema: { default: "" },
  init: function () {
    this.el.addEventListener("click", function () {
      AFRAME.scenes[0].emit("delPortals", null);
  ...
  init: function () {
    ...
    if (isHMD === true) {
      ...
      window.addEventListener("enter-vr", (e) => {
        ...
        rightCon.setAttribute(
          "teleport-controls",
          "button: trigger; collision-entities: #env; cameraRig: #player; teleportOrigin: #head;"
        );
...
function setImage(username) {
  ...
  if (c !== "") {
    ...
    hive.api.getAccounts([username], (err, result) => {
      ...
      let jsonData = JSON.parse(user.json_metadata);
      let profileData = jsonData.profile;
      AFRAME.scenes[0].emit("setLoggedInImage", {
        val: profileData.profile_image,
      });
...
function getAccountInfo(username) {
  hive.api.getDynamicGlobalProperties((err, result) => {
    ...
    hive.api.getAccounts([username], (err, result) => {
      ...
      let hivePower = hive.formatter.vestToHive(
        vestingShares,
        totalVestingShares,
        totalVestingFundHive
      );
      let delegatedHivePower = hive.formatter.vestToHive(
        receivedVestingShares.split(" ")[0] + " VESTS",
        totalVestingShares,
        totalVestingFundHive
      );
      let outgoingHivePower =
        hive.formatter.vestToHive(
          receivedVestingShares.split(" ")[0] -
            delegatedVestingShares.split(" ")[0] +
            " VESTS",
          totalVestingShares,
          totalVestingFundHive
        ) - delegatedHivePower;
      // vote power calc
      let lastVoteTime =
        (new Date() - new Date(user.last_vote_time + "Z")) / 1000;
      ...
      hive.api.getFollowCount(user.name, function (err, result) {
        ...
        hive.api.getAccounts([user.name], (err, result) => {
          ...
          let data = {
            ...
            effectiveSp: parseInt(
              hivePower + delegatedHivePower - -outgoingHivePower
            ),
...
function setPortals(action) {
  ...
  for (let i = 0; i < action.length; i++) {
    ...
    hive.api.getAccounts([post.author], (err, result) => {
      ...
      if (
        Math.round(
          parseFloat(post.pending_payout_value.substring(0, 5)) * 100
        ) /
          100 >
        0
      ) {
        ...
        val =
          Math.round(
            parseFloat(post.total_payout_value.substring(0, 5)) * 100
          ) / 100;
      }
...
AFRAME.registerState({
  initialState: {
    loggedIn: false,
    loggedInImage:
      "https://cdn.glitch.com/5ba0e9a1-e1be-470c-be6c-b6bd1b8e349e%2FOTOLUX%20Tag.png?1528445998829",
