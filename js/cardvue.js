                              ...
                              <div class="collapse" :id="'contract-' +  post.author + '-' + post.permlink">
                                  <form v-for="(cid, name, index) in post.contract" id="contractForm">
                                      ...
                                      {{expIn(contracts[name])}}</span></div>
                                        <a role="button" @click="showNodes = !showNodes"><h6>Storage Detail ({{contract.nt}}/{{contract.p}})</h6></a>
                                      <div v-if="showNodes" v-for="acc in contract.n">
                                        <p>@{{acc}}</p>
                                        </div>
                                        <a v-if="has_ipfs" role="button" @click="store(contract.i, isStored(contract.i))"><h6>{{isStored(contract.i) ? 'Remove from Storage' : 'Place in Storage'}}</h6></a>
